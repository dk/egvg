...
uses Objects, Drivers, Views, EgInline;
...
const
  {COutlineViewer = CListViewer;}
  COutlineViewer = #10#9#12#13#14#15#16#17;
...
{ TOutlineViewer  object }
  ...
  TOutlineViewer = object(TScroller)
    ...
    procedure Update;
    procedure DrawString(Cur: Pointer; R: TRect; S: String; Color: integer);virtual; {+ by SHIM}
  private
...
procedure TOutlineViewer.AdjustFocus(NewFocus: Integer); {SHIM}
var
  OldFocus : integer;
  R        : TRect;
begin
  ...
  else if NewFocus - (Size.Y div Step.Y) >= Delta.Y then
    ScrollTo(Delta.X, (NewFocus + 1) - Size.Y div Step.Y)
  else begin
    if OldFocus < NewFocus then ExchangeWords(word(OldFocus), word(NewFocus));
    R.A.Y := (NewFocus - Delta.Y) * Step.Y;
    R.B.Y := (OldFocus - Delta.Y + 1) * Step.Y;
  end;
  InvalidateRect(R);
end;

{ Called to draw the outline }
procedure TOutlineViewer.DrawString(Cur: Pointer; R: TRect; S: String; Color: integer); {+ by SHIM}
begin
  Bar(R.A.X, R.A.Y, R.B.X, R.B.Y, Lo(Color));
  WrCStr(R.A.X, R.A.Y, S, Hi(Color));
end;
...
var
  ...
  begin
    ...
    begin
      if Position >= Delta.Y + Size.Y div Step.Y then {SHIM}
      begin
        DrawTree := True;
        Exit;
      end;

      R.Assign(0, (Position - Delta.Y) * Step.Y, Size.X, (Position - Delta.Y + 1) * Step.Y);{SHIM}
      RE.Copy(R);
      RE.Intersect(Inv);
      if RE.Empty then Exit;
      ...
      if (Position = Foc) and (State and sfFocused <> 0) then
        ...
        Color := NrmColor;
      {MoveChar(B, ' ', Color, Size.X);} {SHIM}
      S := GetGraph(Level, Lines, Flags);
      if Flags and  ovExpanded = 0 then
        S := Concat(S, '~', GetText(Cur), '~')
      else
        S := Concat(S, GetText(Cur));
      {MoveCStr(B, Copy(S, Delta.X + 1, 255), Color);}  {SHIM}
      {WriteLine(0, Position - Delta.Y, Size.X, 1, B);} {SHIM}
      DrawString(Cur, R, Copy(S, Delta.X + 1, 255), Color);{SHIM}
      I := Position;
    end;
  end;

begin
  GetInvalidExtent(Inv);
  NrmColor := GetColor($0102);
  SelColor := GetColor($0304);
  FocColor := GetColor($0506);
  FirstThat(@DrawTree);
  {MoveChar(B, ' ', NrmColor, Size.X);} {SHIM}
  {WriteLine(0, I + 1, Size.X, Size.Y - (I - Delta.Y), B);} {SHIM}
  Bar(0, (I - Delta.Y + 1) * Step.Y, Size.X, Size.Y, Lo(NrmColor)); {SHIM}
end;
      ...
      begin
        ...
        repeat
          ...
          if MouseInView(Event.Where) then
            NewFocus := Delta.Y + Mouse.Y div Step.Y {SHIM}
          else
          begin
        ...
        begin
          ...
          begin
            ...
            if (Mouse.X div Step.X) < Length(Graph) then
            begin
    ...
    evKeyboard:
      begin
        ...
        case CtrlToArrow(Event.KeyCode) of
          ...
          kbPgDn:          Inc(NewFocus, Size.Y div Step.Y - 1);
          kbPgUp:          Dec(NewFocus, Size.Y div Step.Y - 1);
          kbHome:          NewFocus := Delta.Y;
          kbEnd:           NewFocus := Delta.Y + Size.Y div Step.Y - 1;
