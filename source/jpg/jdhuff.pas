...
{ Original: jdhuff.h+jdhuff.c;  Copyright (C) 1991-1996, Thomas G. Lane. }
...
type
  ...
  d_derived_tbl = record
    ...
    mincode : array[0..17-1] of INT32;       { smallest code of length k }
...
type
  bitread_perm_state = record   { Bitreading state saved across MCUs }
    ...
    printed_eod : boolean;      { flag to suppress multiple warning msgs }
  end;
...
type
  bitread_working_state = record
    ...
    unread_marker : int;        { nonzero if we have hit a marker }
    ...
    printed_eod_ptr : ^boolean; { => flag in permanent state }
  end;
...
begin
  ...
  begin
    ...
    jpeg_make_d_derived_tbl(cinfo, cinfo^.dc_huff_tbl_ptrs[dctbl],
			    {var} entropy^.dc_derived_tbls[dctbl]);
    jpeg_make_d_derived_tbl(cinfo, cinfo^.ac_huff_tbl_ptrs[actbl],
			    {var} entropy^.ac_derived_tbls[actbl]);
