{TONY}
{$IFDEF Windows}
  !!! This unit does not work under MS Windows environment !!!
{$ENDIF}
{$IFNDEF Ver70}
...
const
  ...
  begin
    ...
    MergeIntoPicture(Picture, NameOnly,    Month);
    MergeIntoPicture(Picture, WeekDayOnly, DOW);
...
{$ELSE}
      ...
      R.dx := 0;
      R.ds := HiLo(L).HiWord;
      if SimulateRealModeInt($21, R) = 0 then begin
        GetCountryInfo := True;
        Move(Ptr(HiLo(L).LoWord, 0)^, Info, 34);
      end
      else
