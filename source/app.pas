...
{ CBackGround            ‚ïë 1 ‚ïë }
{                        ‚ïö‚ïê‚ï§‚ïê‚ïù }
{ Back ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îò   }
...
{$IFDEF Ukrainian}
  New(StatusLine, Init(R,
    NewStatusDef(0, $FFFF,
      NewStatusKey('~Alt-X~ ÇÎÂÆ§', kbAltX, cmQuit,
...
{$IFDEF Ukrainian}
  StdFileMenuItems :=
    NewItem('~ç~Æ¢Î©', '', kbNoKey, cmNew, hcNew,
    NewItem('~é~‚™‡Î‚Ï...', 'F3', kbF3, cmOpen, hcOpen,
    NewItem('~ë~ÆÂ‡†≠®‚Ï', 'F2', kbF2, cmSave, hcSave,
    NewItem('~è~•‡•®¨•≠Æ¢†‚Ï...', '', kbNoKey, cmSaveAs, hcSaveAs,
    NewItem('ëÆÂ‡†≠®‚Ï ~¢~·•', '', kbNoKey, cmSaveAll, hcSaveAll,
    NewLine(
    NewItem('ë¨•≠®‚Ï ~§~®‡•™‚Æ‡®©...', '', kbNoKey, cmChangeDir, hcChangeDir,
    NewItem('Ç~‡~•¨•≠≠Î© ¢ÎÂÆ§', '', kbNoKey, cmDosShell, hcDosShell,
    NewItem('Ç~Î~ÂÆ§', 'Alt+X', kbAltX, cmQuit, hcExit,
    Next)))))))));
{$ELSE}
  {$IFDEF Russian}
  StdFileMenuItems :=
...
{$IFDEF Ukrainian}
  StdEditMenuItems :=
    NewItem('~é~‚™†‚', '', kbAltBack, cmUndo, hcUndo,
    NewLine(
    NewItem('~Ç~Î‡•ß†‚Ï', 'Shift+Del', kbShiftDel, cmCut, hcCut,
    NewItem('ë~™~ÆØ®‡Æ¢†‚Ï', 'Ctrl+Ins', kbCtrlIns, cmCopy, hcCopy,
    NewItem('ÇÆ~·~·‚†≠Æ¢®‚Ï', 'Shift+Ins', kbShiftIns, cmPaste, hcPaste,
    NewItem('é~Á~®·‚®‚Ï', 'Ctrl+Del', kbCtrlDel, cmClear, hcClear,
    Next))))));
{$ELSE}
  {$IFDEF Russian}
  StdEditMenuItems :=
...
{$IFDEF Ukrainian}
  StdWindowMenuItems :=
    NewItem('è†‡™•~‚~', '', kbNoKey, cmTile, hcTile,
    NewItem('~ä~†·™†§', '', kbNoKey, cmCascade, hcCascade,
    NewItem('~á~†™‡Î‚Ï ¢·•', '', kbNoKey, cmCloseAll, hcCloseAll,
    NewLine(
    NewItem('~ê~†ß¨•·‚®‚Ï','Ctrl+F5', kbCtrlF5, cmResize, hcResize,
    NewItem('ê~†~ß¨•‡', 'F5', kbF5, cmZoom, hcZoom,
    NewItem('~ë~´•§„ÓÈ••', 'F6', kbF6, cmNext, hcNext,
    NewItem('~è~‡•§Î§„È••', 'Shift+F6', kbShiftF6, cmPrev, hcPrev,
    NewItem('~á~†™‡Î‚Ï', 'Alt+F3', kbAltF3, cmClose, hcClose,
    Next)))))))));
{$ELSE}
  {$IFDEF Russian}
  StdWindowMenuItems :=
