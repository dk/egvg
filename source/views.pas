...
{ TView PalMode modes }
  pmUseRGB    = $001;   {включается в процесс перерисовки при cмене палитры}
  pmOptimize  = $002;   {включается в процесс оптимизации цветов}
  pmDirectBM  = $004;   {отключен авто вывод BMP через ColorRef}
  pmMonopoly  = $008;   {запрет на деление цветов с другими палитрами}
  pmInHeap    = $010;   {палитра в куче а не в коде}
  pmEnable    = $020;   {используется внутри - не ставить}
  pmDrawFirst = $040;   {используется внутри - не ставить}
  pmKeepFocus = $080;   {если получает фокус переделывает палитру}
  pmHiColor   = $100;   {форсирует использование старших моделей палитры}
  pmComplex   = $200;   {установить для смешанного рисования - битмар с палитрой/текст и пр.}
...
{кто использует pmDirectBM должен перекрыть UpdateColors чтобы видоизменить
 свои цветовые таблицы. Этот эффект может быть использован для обоев,
 которые при UpdateColors 1 раз пересчитаются а дальше будут выводиться
 без перекодировки по PaintInfo.ColorRef, что несколько быстрее.
 PS - ясное дело только для 256 цветов}
  ...
  if BufferedStrategy then begin
    ...
    if DrawLockCounter=0 then begin
      ...
      PutBufferPart(RefreshRegion.A.X, RefreshRegion.A.Y,
                   RefreshRegion.B.X, RefreshRegion.B.Y);{ - заменен на unscrew.}
      HoldMouse(False);
...
begin
  ...
  Bounds.B.Y := Bounds.A.Y + Range(Bounds.B.Y - Bounds.A.Y, Min.Y, Max.Y);
  GetBounds(R);
(*  if not Bounds.Equals(R) then {GIO - ошибка при движении}
  begin*)
  ChangeBounds(Bounds);
...
type
  ...
  TLPM = record
    ...
    Command  : Word;
    InfoPtr  : Pointer;
    SentAt   : Word; { Время, когда отсылать событие }
  end;
...
CanGiveQuantum:
    ...
    if MultiCount = 255 then
      ...
      LPMCanBeSent := False; { По-видимому, нельзя отдавать квант ! }
  end;
end;
