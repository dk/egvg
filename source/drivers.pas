...
procedure ReStr(var P: PString; const S: String); {TONY}
  {--- Сначала DisposeStr(P) затем P:= NewStr(S) }
...
var
  ...
  {без этого малопонятные лажи под OS/2 с DPMI - такое впечатление
  что VMOUSE для DOS-Box'a дает совсем мало стека на хук int 33}
...
var  {TONY}
  ...
  MouseCounter: integer;   { >=0 - видимо, <0 - невидимо }
...
{$IFDEF Russian}
    PrintStr('Ошибка инициализации графики');
{$ELSE}
...
asm
        ...
        LODSW
        OUT     DX, AX
        { Смешаный регистр (6); Разрешена адресация к знакоген,       }
        { адрес A000 для 64 K (4)                                     }
        LODSW
        OUT     DX, AX
        { Регистр выбора карты чтения (4); Плоскость 2 (?)            }
        LODSW
        OUT     DX, AX
        { Собственно загрузка второй плоскости }
        PUSH    SI
        PUSH    DS
        MOV     AL, [Count]
        MOV AH, [Height]
...
begin
  ...
  LoadCharacterGenerator(CharHeight, 'А', 128, FontPtr);
end;
...
{$IFDEF Ukrainian}
  SCriticalErrorHeader: string[20] = 'Критическая ошибка !';   {TONY}
  SCriticalError:  string[29] = 'Критический сбой дисковода %c';
  SWriteProtected: string[25] = 'Диск %c защищен от записи';
  SDiskNotReady:   string[20] = 'Дисковод %c не готов';
  SDataIntegrity:  string[27] = 'Ошибка в данных на диске %c';
  SSeekError:      string[32] = 'Ошибка позиционирования диска %c';
  SUnknownMedia:   string[35] = 'Неизвестный носитель в дисководе %c';
  SSectorNotFound: string[28] = 'Не найден сектор на диске %c';
  SOutOfPaper:     string[21] = 'Нет бумаги в принтере';
  SWriteFault:     string[24] = 'Ошибка записи на диск %c';
  SReadFault:      string[22] = 'Ошибка чтения диска %c';
  SGeneralFailure: string[28] = 'Аппаратный сбой дисковода %c';
  SBadImageOfFAT:  string[31] = 'Неправильная копия FAT в памяти';
  SDeviceError:    string[27] = 'Ошибка доступа к устройству';
  SInsertDisk:     string[27] = 'Вставьте диск в дисковод %c';
  SRetryOrCancel:  string[30] = '<Enter> Повторить  <Esc> Отказ';
{$ELSE}
  {$IFDEF Russian}
  SCriticalErrorHeader: string[20] = 'Критическая ошибка !';   {TONY}
  SCriticalError:  string[29] = 'Критический сбой дисковода %c';
...
begin
  ...
  if Font^.Width <> 8 then begin
    RunError(255);  { Неверный системный шрифт }
  end;
  ...
  XEnd := XBeg + XSize;
  YBeg := (ScreenHeight - YSize) div 3;  { Первая треть экрана }
  YEnd := YBeg + YSize;
...
{$IFDEF Ukrainian}
  AltCodes1: array[$10..LastAltCode] of Char =
    'йцукенгшщзх°'#0#0'фЎвапролджЇ'#0#0#0'ячсмитьбюЄ';
{$ELSE}
  {$IFDEF Russian}
  AltCodes1: array[$10..LastAltCode] of Char =
    'йцукенгшщзхъ'#0#0'фывапролджэ'#0#0#0'ячсмитьбюё';
  {$ELSE} {English}
  AltCodes1: array[$10..LastAltCode] of Char =
