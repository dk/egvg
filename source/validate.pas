...
{$IFDEF Ukrainian}
  StrPCopy(StrECopy(MsgStr,
    'Ввод не соответствует образцу:'#10'    '), Pic^);
{$ELSE}
  {$IFDEF Russian}
  StrPCopy(StrECopy(MsgStr,
    'Ввод не соответствует образцу:'#10'    '), Pic^);
...
begin
{$IFDEF Ukrainian}
  MessageBox('Ввод не соответствует образцу:'#13' %s', @Pic,
    mfError + mfOKButton);
{$ELSE}
  {$IFDEF Russian}
  MessageBox('Ввод не соответствует образцу:'#13' %s', @Pic,
...
begin
{$IFDEF Ukrainian}
  MessageBox(0, 'Введен неверный символ', 'Validator', mb_IconExclamation or mb_Ok);
{$ELSE}
  {$IFDEF Russian}
  MessageBox(0, 'Введен неверный символ', 'Validator', mb_IconExclamation or mb_Ok);
  {$ELSE}
...
begin
{$IFDEF Ukrainian}
  MessageBox('Введен неверный символ', nil, mfError + mfOKButton);
{$ELSE}
  {$IFDEF Russian}
  MessageBox('Введен неверный символ', nil, mfError + mfOKButton);
  {$ELSE}
...
begin
  inherited Init(['0'..'9','+','-']);
  Options:= Options or voTransfer; {OOA}
...
begin
  ...
  Params[1] := Max;
{$IFDEF Ukrainian}
  wvsprintf(MsgStr, 'Значение должно быть от %ld до %ld.', Params);
{$ELSE}
  {$IFDEF Russian}
  wvsprintf(MsgStr, 'Значение должно быть от %ld до %ld.', Params);
  {$ELSE}
...
begin
  ...
  Params[1] := Max;
{$IFDEF Ukrainian}
  MessageBox('Значение должно быть от %d до %d', @Params,
    mfError + mfOKButton);
{$ELSE}
  {$IFDEF Russian}
  MessageBox('Значение должно быть от %d до %d', @Params,
...
begin
{$IFDEF Ukrainian}
  MessageBox(0, 'Ввод неверного значения', 'Validator',
...
begin
{$IFDEF Ukrainian}
  MessageBox('Ввод неверного значения', nil, mfError + mfOKButton);
{$ELSE}
  {$IFDEF Russian}
  MessageBox('Ввод неверного значения', nil, mfError + mfOKButton);
  {$ELSE}
